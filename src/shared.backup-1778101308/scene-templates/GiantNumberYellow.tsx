 ...
 *
 * Visual: BG yellow + radial glow + giant 480px number with coral shadow + coral pill + quote.
 *
 * Example:
 *   <GiantNumberYellow
 *     pillar="a"
...
export const GiantNumberYellow: React.FC<{
  pillar?: PillarKey;
  number: string;           // '800'
  unit?: string;            // 'NĂM TUỔI'
  attribution?: string;     // '— Vũ Bằng, "Miếng ngon Hà Nội" 1957'
  numberSize?: number;      // default 480
  shadowColor?: string;     // default coral
  bg?: string;              // default brand.yellow (override khi cần)
}> = ({
  pillar = 'a',
  number,
  unit,
  ...
  return (
    <AbsoluteFill style={{ background: bg ?? color.brand.yellow }}>
      ...
      <AbsoluteFill style={{
        ...
        {/* Giant number with coral shadow */}
        <div style={{
