...
export const RatioCells: React.FC<{
  ...
  extrasHighlight?: string | string[]; // 'ớt' hoặc ['vị béo','mềm','đậm đà'] — multi highlight
  photoSrc?: string;
      ...
      <AbsoluteFill style={{
        ...
        {extras && (
          <div style={{
            ...
            {(() => {
              const highlights = Array.isArray(extrasHighlight)
                ? extrasHighlight
                : extrasHighlight ? [extrasHighlight] : [];
              if (!highlights.length) return extras;
              const escaped = highlights.map(h => h.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|');
              const re = new RegExp(`(${escaped})`, 'g');
              return extras.split(re).map((part, i) =>
